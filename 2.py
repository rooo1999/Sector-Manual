...
def calculate_trailing_returns(series):
    """Calculates trailing returns from a periodic (non-daily) time series."""
    returns = {}
...
def style_table(styler, format_str, na_rep, cmap, vmin, vmax, weight_col=None):
    ...
    styler.background_gradient(cmap=cmap, subset=cols, vmin=vmin, vmax=vmax)
    if na_rep == "None":
...
if run_button and uploaded_file:
    ...
    if not all_portfolios_data:
        st.error("No valid portfolio data found. Please check the Excel file format.")
    else:
        # --- UPDATED BENCHMARK CODES ---
        BENCHMARKS = {
            "Nifty 50 TRI": "147794",
        ...
        portfolio_results = {}
        with st.spinner("Processing all portfolios..."):
        ...
        with tabs[0]:
            ...
            st.dataframe(style_table(comparison_df.style, '{:.2%}', 'N/A', excel_cmap, -0.05, 0.05), use_container_width=True)
        ...
        for i, (name, results) in enumerate(portfolio_results.items()):
            with tabs[i+1]:
                ...
                benchmark_value_indices = (1 + benchmark_periodic_returns).cumprod(axis=1).fillna(1)
                benchmark_value_indices = benchmark_value_indices.apply(lambda x: x * initial_investment if x.name in benchmark_periodic_navs.index else x, axis=0)
                ...
                cols_order = ['Weight', '1 Month', '3 Months', '6 Months', '1 Year', '3 Years', '5 Years']
                final_cols_trailing = [c for c in cols_order if c in fund_trailing_returns.columns]
                st.dataframe(style_table(fund_trailing_returns[final_cols_trailing].style, '{:.2%}', 'N/A', excel_cmap, -0.05, 0.05, 'Weight'), use_container_width=True)
                ...
                st.dataframe(style_table(combined_trailing[final_cols_trailing_bench].style, '{:.2%}', 'N/A', excel_cmap, -0.05, 0.05), use_container_width=True)
                ...
                df_fund_periodic['Weight'] = allocations.iloc[:, -1]
                st.dataframe(style_table(df_fund_periodic.style, '{:.2f}', 'None', excel_cmap, -8, 8, 'Weight'), use_container_width=True)

                st.subheader("✅ Portfolio vs Benchmarks (Periodic Returns)")
                # Correctly calculate portfolio periodic returns from its value index
                ...
                st.dataframe(style_table(combined_periodic.style, '{:.2f}', 'None', excel_cmap, -8, 8), use_container_width=True)
